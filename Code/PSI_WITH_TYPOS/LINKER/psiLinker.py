...
def hello_world():
    if request.method == 'POST':
        ...
        if (data[0][0] == 'QUERY'):
            ...
            num_obs = dfP1.shape[0] 
            print(num_obs)

            num_obs = dfP2.shape[0] 
            print(num_obs)

            # create one joined dataframe from both dataframes, join on Key columns
            block_DataIntersection = pd.merge(
                dfP2, dfP1, how='inner', left_on=block_KeyP2, right_on=block_KeyP1)
            num_pairs = block_DataIntersection.shape[0] 
            print(num_pairs)
            # print(link_DataIntersection)
            
            link_DataIntersection = pd.merge(
                dfP2, dfP1, how='inner', left_on=link_KeyP2, right_on=link_KeyP1)
            num_pairs = link_DataIntersection.shape[0] 
            print(num_pairs)
            # print(link_DataIntersection)
            ...
            block_resultdf = block_DataIntersection.groupby(
                query_columns).count()[block_KeyP1].reset_index()

            print(block_resultdf)
            ...
            link_resultdf = link_DataIntersection.groupby(
            ...
            block_columns = block_resultdf.columns
            # print(block_columns.values)

            link_columns = link_resultdf.columns

            # create response message containing: LINKER, Column names, groupedby values (all space separated)
            block_msglist = block_resultdf.values.tolist()
            block_m = FromList2Mess(block_msglist, 'LINKER', block_columns)
            link_msglist = link_resultdf.values.tolist()
            link_m = FromList2Mess(link_msglist, 'LINKER', link_columns)
            m = block_m+"|"+link_m
            # print(str(m))
