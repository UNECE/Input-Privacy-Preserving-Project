...
def process_request():
    ...
    if request.method == 'POST':
        ...
        for i in range(len(split_vals)):
            client_elements.append(int(split_vals[i]))
        ...
        if link_type == 0:
            hashed_server_keys = [hash_int(i) for i in server_elements_b]
        else:
            hashed_server_keys = [hash_int(i) for i in server_elements_l]
...
if __name__ == '__main__':
    ...
    fin = open(fileName, "r")
    server_elements_b = [row.split(";")[0] for row in fin]
    fin.close()

    fin = open(fileName, "r")
    server_elements_l = [row.split(";")[1] for row in fin]
    fin.close()

    # convert to string
    for i in range(len(server_elements_b)):
        server_elements_b[i] = str(server_elements_b[i])
        
    for i in range(len(server_elements_l)):
        server_elements_l[i] = str(server_elements_l[i])
